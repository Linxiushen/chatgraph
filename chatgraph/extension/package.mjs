...
const destination=process.argv[2]?resolve(process.argv[2]):fileURLToPath(new URL('../test-output/chatgraph-extension-0.2.0.zip',import.meta.url));
const temporary=await mkdtemp(join(tmpdir(),'chatgraph-extension-'));
try{
  const archive=join(temporary,'extension.zip');
  const files=['manifest.json','extractor.js','popup.html','popup.css','popup.js','README.md'].map(file=>join(directory,file));
