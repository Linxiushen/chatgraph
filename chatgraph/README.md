...
**0.4.2-beta.1** 是个人工作区与手机 App 测试版，本轮加固付费任务幂等、登录恢复、跨设备删除保护、原生收件与全量备份。提供从对话整理、图谱编辑到知识积累与分享的完整流程。采用 MIT 许可证，第三方来源与复用范围见 [上游与技术说明](UPSTREAM.md)。
...
点击“使用选中的对话”后，只有选定范围进入导入草稿；点击“开始整理”才发送到服务端，AI 模式随后发送给配置的模型。其他会话不会写入知识库或导入草稿。直接在编辑器选择的文件随窗口关闭释放；从手机收件箱接收的原件会临时保留在该浏览器的收件箱，成功整理、手动删除或到期后清除。

超过消息数或请求大小时，预览会明确显示初始选中的部分范围，可修改后再次核对。跳过图片、工具消息或其他分支时给出说明；不会通过交替排列猜测发言者。单条消息最多 100,000 字符，超长消息需先拆分，或选择不包含它的范围。

## 平台入口与在线部署

**手机从 `/mobile-inbox.html` 进入。** 提供可安装到主屏幕的 PWA、手机粘贴/文件收件箱、兼容 Android 浏览器的系统分享目标，以及 iPhone Safari 快捷指令采集脚本。收件箱最多保留 5 条、总计 50 MB、24 小时，内容只存在接收它的浏览器中，可能被系统提前清理。分享链接只记录出处，不自动抓取登录后的聊天。离线可暂存导入，生成与知识库访问需要连接服务。

[手机接入与权限说明](docs/mobile-platforms.md) · [iPhone Safari 快捷指令配置](mobile/README.md)。另外提供 [Android 原生 App](mobile/android/README.md) 和 [iOS App 与分享扩展](mobile/ios/README.md)：Android 是 debug 测试签名 APK，iOS 是未签名 IPA 与 Xcode 工程，需要开发者自行签名。尚未进行真实手机验收或上架；均不具备后台读取其他原生 App 聊天的权限。Safari、原生 WebView、浏览器标签页与已安装 PWA 的存储可能隔离，应在同一个入口里完成接收和导入。

手机需能访问一个已部署的 **HTTPS 工作区**，按下方部署文档配置。电脑上的 `127.0.0.1:4317` 只指向电脑本机，不能直接填到手机上使用。模型密钥留在服务端。
...
导入只处理文字，不理解图片、语音或附件。最多 500 条原文、200 个节点；原文整理最多 199 条消息。账号文件可在浏览器内本地读取至 25 MB；网页选定原文仍限 2 MiB；规范化图谱 JSON 总量限 8 MiB，普通 HTTP JSON 请求限 32 MiB（包括 JSON 转义和追加时已有图谱），备份恢复限 50 MiB；超限会明确失败，不悄悄丢弃原文。图谱不能容纳新的追加内容时，请另建图谱。AI 关联检索最多扫描 2,500 个节点且候选摘要不超过 900,000 字符。跨对话关系建议发送被引用的完整原文与节点，总负载上限 240,000 个序列化字符；超限在调用前提示按主题拆分，不截断依据。
...
工程故障场景、自动化验收与正式上线的外部条件见 [工程验收矩阵](docs/engineering-readiness.md)。停机完整备份、校验、恢复及容器演练命令见 [部署文档](deploy/README.md)。
