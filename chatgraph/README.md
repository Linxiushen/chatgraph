...
**0.4.0-beta.1** 是个人工作区与手机入口测试版，提供从对话整理、图谱编辑到知识积累与分享的完整流程。采用 MIT 许可证，第三方来源与复用范围见 [上游与技术说明](UPSTREAM.md)。
...
[手机接入与权限说明](docs/mobile-platforms.md) · [iPhone Safari 快捷指令配置](mobile/README.md)。这些是 PWA 与源码入口，不是已经签名上架的 IPA/APK；不具备后台读取其他原生 App 聊天的权限。Safari、浏览器标签页与已安装 PWA 的存储在部分平台可能隔离，应在同一个入口里完成接收和导入。
