...
1. 在手机上解压发布的 Android 压缩包，打开其中的 `ChatGraph-android-0.4.2-beta.1-debug.apk`，按系统提示允许本次安装。
2. 首次启动填写已部署的 **ChatGraph HTTPS 根地址**，例如 `https://graph.example.com`。服务器请使用 0.4.1 或更新版，以支持 App 重启后的任务恢复。示例域名不可直接使用。手机的 `127.0.0.1` 不是电脑上的服务；本包不包含服务器或 API 密钥。
...
JVM 单测使用临时目录验证原文/元数据恢复、旧版备份兼容、并发容量限制、Intent 重放去重及错误输入边界。它不替代真实 Android 的分享面板、文件提供器、进程回收和磁盘断电验收。
