...
export function createShareStore({ dataDir }) {
  ...
  return {
    async create(input) {
      ...
      await fs.writeFile(filename(token), JSON.stringify(share), { mode: 0o600, flag: 'wx' });
      return metadata(share);
    },
    async load(token) {
      let share;
      try { share = JSON.parse(await fs.readFile(filename(token), 'utf8')); }
      catch (error) { if (error.code === 'ENOENT') throw Object.assign(new Error('分享不存在或已撤回。'), { status: 404 }); throw error; }
      if (Date.parse(share.expiresAt) <= Date.now()) throw Object.assign(new Error('分享已过期。'), { status: 410 });
      share.graph = validateGraph(share.graph);
      return share;
    },
    async list() {
      let entries;
      ...
      for (const entry of entries) {
        ...
        try { shares.push(metadata(JSON.parse(await fs.readFile(path.join(directory, entry), 'utf8')))); } catch { /* Do not expose malformed content. */ }
      }
      return shares.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    },
    async remove(token) { await fs.rm(filename(token), { force: true }); return { ok: true }; },
  };
}
