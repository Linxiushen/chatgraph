...
async function requestJSON(prompt, payload, { api, env = process.env, fetchImpl = fetch, signal, repairAttempt = 0 } = {}) {
  ...
  const data = await response.json().catch(() => { throw new Error('模型服务没有返回有效 JSON。'); });
...
export async function suggestRelations(value, options = {}) {
  ...
  const payload = {
    sessions: graph.sessions.map(session => ({ id: session.id, title: session.title, messageIds: session.messageIds, nodeIds: session.nodeIds })),
    ...
    messages: graph.messages.map(message => ({ ...message, content: message.content.slice(0, 2000) })), edges: graph.edges,
  };
  const { result, analysis } = await requestJSON('查找不同 session 之间有原文支持的观点关联，输出 {relations:[{source,target,type,label,explanation,evidenceIds}]}，最多 15 项。type 只能 supports/challenges/revises/depends。revises 必须是后续用户明确改变旧判断，从新指向旧；challenges 可保留双方分歧而不决定真假。evidenceIds 至少包含来自两端观点的原文 ID。不要重复已有关系，不修改节点状态或人工备注。缺乏证据就返回空数组。', payload, options);
  ...
  for (const item of result.relations) {
    ...
    if (!Array.isArray(item.evidenceIds) || !item.evidenceIds.length || item.evidenceIds.some(id => !messages.has(id)) || !item.evidenceIds.some(id => source.sourceIds.includes(id)) || !item.evidenceIds.some(id => target.sourceIds.includes(id))) throw new Error('模型关联建议缺少两端观点的原文依据。');
    if (item.type === 'revises') {
      const latestTarget = Math.max(...target.sourceIds.map(id => messages.get(id).index));
      if (!source.sourceIds.some(id => messages.get(id).role === 'user' && messages.get(id).index > latestTarget)) throw new Error('修正关系缺少后续用户判断依据。');
    }
